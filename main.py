...
#Añade a la lista los archivos en /conocidos
alumnosDes = [
...
def get_module_by_time(entry_time):
    ...
    time_in_minutes = hour * 60 + minute  # Convertir la hora y minutos en minutos totales desde la medianoche

    # Definición de los rangos en minutos totales

    for start, end, module_index in modulosRangos:
        if start <= time_in_minutes < end:
            print2(f'Modulo {module_index} ({modulos[1][module_index]})')
            return f'Modulo {module_index} ({modulos[1][module_index]})'

    # Si no se encuentra un módulo, retornar una cadena vacía
    return ""
...
def plantilla(ws):
    ...
    for index, alumno in enumerate(alumnos, start=1):
        ...
        # Celdas de asistencia y observaciones
        for col in [3, 4]:
...
def add_to_excel(name, entry_time):
    ...
    if module_sheet != "":
        ...
        if sheet.max_row == 3: 
            ...
            #Registrar la hora de asistencia más temprana detectada
            ...
            if name in registered_names and not celda.value:
                ...
                elif modulosRangos[moduloActual-1][0]+10 >= time_in_minutes:
        ...
        #Marcar inasistencias al final del modulo        
